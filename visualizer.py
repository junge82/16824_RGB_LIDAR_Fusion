##TEMPORARY FILE FOR VISUALIZATION. NOT USED IN TRAINING OR EVALUATION. IGNORE ALL CHANGES TO THIS FILE.
...
from utils.kitti_viewer import draw_2d_output, draw_3d_output, show_lidar_with_boxes
...
def get_args(arg_list=None):
    ...
    parser.add_argument('--output_dir', type=str, default="/mnt/fastDisk/kitti3d/kitti_object/training/labels_2")
...
def main(args):
    ...
    for batch_idx, batch_data in enumerate(tqdm(train_loader)):
        ...
        # output_kitti = loss_fn.convert_yolo_output_to_kitti_labels(outputs[0], calibs[0], args.inference_conf_threshold)
        # draw_3d_output(lidars[0].to('cpu').numpy(), labels[0].numpy().tolist(), calibs[0], output_kitti.detach().to('cpu').numpy().tolist())
        # resizer = transforms.Resize((args.ori_img_h, args.ori_img_w), antialias=True)
